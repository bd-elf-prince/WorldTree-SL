...
recoverPlane() {
    vector a = posOf(1);
    vector b = posOf(6);
    if (linkOf(1) > 0 && linkOf(6) > 0 && llFabs(a.z-b.z) <= 0.05) {
        cardPlane = (a.z+b.z)*0.5;
    }
    else if (linkOf(1) > 0) cardPlane = a.z;
    else if (linkOf(6) > 0) cardPlane = b.z;
...
initUI() {
    ...
    recoverPlane();
    restoreCards();
    llOwnerSay("VIEW v6.5.7 ready. memory="+(string)llGetFreeMemory());
}
