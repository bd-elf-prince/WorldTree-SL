/* WT_HUD_View v6.5.9-z-only-fix
 * Based on v6.5.7.
 * Fix: only card local position Z is forced.
 * Does not change PRIM_SIZE / baseSizes.
 * Memory-reduced card test build.
 * - Normal cards stay at full brightness.
 * - Held card fades 100%→50% for 1s, then 50%→100% for 1s.
 * - Release immediately restores full brightness.
 * - Smooth eased close/open flip.
 * - Explicit tree/angel/demon textures.
 */
...
recoverPlane() {
    // Z-only correction. Do not modify card sizes.
    // The previous patch touched PRIM_SIZE/baseSizes and caused card scale drift.
    cardPlane = CARD_Z;
...
initUI() {
    ...
    recoverPlane();
    restoreCards();
    llOwnerSay("VIEW v6.5.9-z-only-fix ready. cardZ="+(string)CARD_Z+" memory="+(string)llGetFreeMemory());
}
...
/* End WT_HUD_View v6.5.9-z-only-fix */
